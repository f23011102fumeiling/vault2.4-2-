            ...
            {['single_choice', 'choice', 'judgment', 'judgement'].includes(q.type) && (q.options?.length || 0) > 0 ? (
              <div className="space-y-2">
                {(Array.isArray(q.options) ? q.options : []).map((opt: any) => {
                  ...
                  return (
                  <label key={displayValue} className="flex items-center gap-2 cursor-pointer">
                    <input
                      type="radio"
                      name={q.id}
                      value={displayValue}
                      checked={(answers[q.id] as string) === displayValue}
              ...
              <div className="space-y-2">
                {(Array.isArray(q.options) ? q.options : []).map((opt: any) => {
                  ...
                  const checked = arr.includes(displayValue)
                  return (
                    <label key={displayValue} className="flex items-center gap-2 cursor-pointer">
                      <input
                        type="checkbox"
                        checked={checked}
                        onChange={() => {
                          const next = checked ? arr.filter((x) => x !== displayValue) : [...arr, displayValue]
                          setAnswer(q.id, next)
                        }}
