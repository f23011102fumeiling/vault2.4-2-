...
async def get_survey_results(survey_id: str, db: Session = Depends(get_db)):
    ...
    try:
        ...
        survey = db.query(Survey).filter(Survey.id == survey_id).first()
        if not survey:
            print(f"❌ 问卷不存在: survey_id={survey_id}")
            raise HTTPException(status_code=404, detail="问卷不存在")
        
        print(f"✅ 找到问卷: {survey.title}")
        ...
        for r in responses:
            print(f"  - response_id={r.id}, student_id={r.student_id}, status={r.status}, total_score={r.total_score}")
        ...
        print(f"📊 统计结果: total_responses={total_responses}, avg_score={avg_score}, pass_count={pass_count}")
        ...
        print(f"✅ 查询成功: totalResponses={result['totalResponses']}")
        print(f"=" * 70)
